...
class BaseSFTModel(SFTModel, ABC):
    ...
    def __init__(self, persona: Persona, num_turns: int):
        ...
        self.messages: list[ChatMessage] = [
            ChatMessage(
                role=Role.SYSTEM, content=self._get_system_prompt(persona, num_turns)
            )
        ]
...
def default_fact_validation(
    facts_to_check: list[Fact], memory_path: str = MEMORY_PATH
) -> bool:
    """
    ...
    **kwargs,
    ...
    async def process_persona(kb_item) -> int:
        ...
        async with persona_semaphore:
            ...
            fact_tasks = [process_fact_for_persona(persona, fact) for fact in facts]

            if fact_tasks:
                # Use tqdm for progress tracking at the fact level
                fact_results = []
                with tqdm(
                    total=len(fact_tasks),
    ...
    task_desc = (
        f"Processing personas for {task_name}" if task_name else "Processing personas"
    )
    ...
    if persona_tasks:
        ...
        print(
            f"Completed {task_name}: {total_successful_facts} successful conversations generated"
        )
