...
def generate_fact_update(persona: Persona, fact: str) -> FactUpdate:
    """
    ...
    """
    prompt = UPDATE_GEN_PROMPT.format(persona=persona, fact=fact)
    response = get_model_response(
        prompt=prompt, model=OPENROUTER_SONNET, schema=FactUpdate
    )
...
class UpdateModel(BaseSFTModel):
    ...
    def _get_system_prompt(self, persona: Persona, num_turns: int) -> str:
        return SFT_PROMPT.format(
            persona=persona, fact_update=self.fact_update, num_turns=num_turns
        )
...
async def generate_convo_for_persona_and_update(
    ...
    update_model = UpdateModel(
        persona=persona, fact_update=fact_update, num_turns=num_turns
    )
...
class UpdateSFTCache:
    ...
    def get_or_create_fact_update(
        self, persona: Persona, fact: Fact
    ) -> Optional[FactUpdate]:
...
def _setup_static_memory_with_cache(
    cache: UpdateSFTCache, persona: Persona, fact: Fact, memory_path: str, **kwargs
):
...
async def generate_update_sft(
    ...
    async def conversation_func_with_cache(
        ...
        return await _generate_update_conversation_with_cache(
            cache, persona, fact, num_turns, validation_func, memory_path, save_folder
        )
