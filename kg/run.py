...
from kg.configs import CONFIGS
...
def generate_retrieval_data(graph: nx.MultiDiGraph, reformatter: QuestionReformat, start_node_id: str, user_md: str,
                            num_qa: int) -> dict:
    """
...
def _calculate_update_diff(original_graph: nx.MultiDiGraph, updated_graph: nx.MultiDiGraph, base_node_id: str,
                           update_info: dict) -> str:
    ...
    # Case 2: A relationship was changed, involving adding/removing nodes/edges.
        ...
        changed_node_slug = original_graph.nodes[update_info["changed_node_id"]]["name"].lower().replace(" ", "_")
        ...
        new_content2 = new_entity_files.get(added_node_slug, {}).get("entity_file_content", "")
        file_path2 = new_entity_files.get(added_node_slug, {}).get("entity_file_path", "")
        diff2 = diff_strings("", new_content2)  # Diff against an empty string for a new file
...
def generate_update_data(driver: KGBuildDriver, reformatter: QuestionReformat, base_node_id: str) -> dict:
    """
...
def process_person_node(driver: KGBuildDriver, reformatter: QuestionReformat, node_id: str, instance_path: Path,
                        cfg: dict):
    """
...
def run(CONFIG):
    ...
    person_nodes = [
        n for n, d in driver.kg.g.nodes(data=True) if d.get("type", "").lower() == NODE_TYPE_PERSON
    ]

    if len(person_nodes) < CONFIG["num_iter_per_graph"]:
        print(f"⚠️ Warning: Not enough person nodes ({len(person_nodes)}) to meet "
